...
SELECT COUNT(DISTINCT (q.raceId)) quali_data
	,t.total_races
	,r.year
FROM qualifying q
JOIN races r ON r.raceId = q.raceId
JOIN (
	SELECT COUNT(DISTINCT (raceId)) total_races
		,year
...
RETURNS NUMERIC(9, 3) DETERMINISTIC
...
SELECT w.driverRef
	...
	,w.track
	,w.won
	,w.times_entered_race
	,ROUND(w.won / w.times_entered_race * 100, 2) percent_win
FROM cte_won W
JOIN (
	SELECT name
	FROM races
...
JOIN (
	SELECT q2022.driverRef
		,r.year
	FROM qualifying_2022 q2022
	JOIN races r ON r.raceId = q2022.raceId
	GROUP BY r.year
		,q2022.driverRef
	) current_year ON current_year.driverRef = w.driverRef
JOIN qualifying_2022 q22 ON q22.driverRef = w.driverRef
WHERE w.won > 5
	AND ROUND(w.won / w.times_entered_race * 100, 2) > 75
	AND w.driverRef != "hulkenberg"
GROUP BY w.track
	,w.driverRef
ORDER BY w.won DESC;
...
WITH cte_win_rate AS (
		WITH min_cte AS (
			...
			,qualis_entered_cte AS (
				SELECT driverId
					,count(raceID) qualis_entered
				FROM qualifying
				GROUP BY driverId
				)
		SELECT d.driverRef
...
WITH cte_min AS (
		SELECT q.position
			...
			,q.driverRef
			,q.constructorId
		FROM qualifying_2022 q
		)

SELECT m.driverRef
	,c.constructorRef
	,COUNT(m.minim) quali_won
FROM cte_min AS m
JOIN constructors c ON c.constructorId = m.constructorId
WHERE m.position = m.minim
	AND m.driverRef != 'hulkenberg'
GROUP BY m.constructorId
	,m.driverRef;
...
CREATE VIEW view_avg_diff_q2_q1_2022
AS
(
		WITH cte_exclude_1 AS (
				SELECT q.raceId exclude
					,AVG(t.diff_q2_q1) average_diff_q2_q1
				FROM qualifying_2022 q
				LEFT JOIN (
					SELECT q2 - q1 diff_q2_q1
						,driverRef
						,raceId
					FROM qualifying_2022
					WHERE q2 != 0
					) t ON t.driverRef = q.driverRef
					AND t.raceId = q.raceId
				GROUP BY exclude
				HAVING average_diff_q2_q1 < - 2
					OR average_diff_q2_q1 > 2
				)
		SELECT c.constructorRef
			,ROUND(AVG(diff_q2_q1), 2) avg_diff_q2_q1
		FROM qualifying_2022 q
		JOIN constructors c ON q.constructorId = c.constructorId
		LEFT JOIN cte_exclude_1 e ON e.exclude = q.raceId
		LEFT JOIN (
			SELECT convert_time(q2) - convert_time(q1) diff_q2_q1
				,driverRef
				,raceId
				,constructorId
			FROM qualifying_2022
			WHERE q2 != 0
			) b ON b.driverRef = q.driverRef
			AND b.raceId = q.raceId
...
AS (
	SELECT q.raceId exclude
		,AVG(t.diff_q3_q2) average_diff_q3_q2
	FROM qualifying_2022 q
	LEFT JOIN (
		SELECT q3 - q2 diff_q3_q2
			,driverRef
			,raceId
		FROM qualifying_2022
		WHERE q3 != 0
		...
		AND t.raceId = q.raceId
	GROUP BY exclude
	HAVING average_diff_q3_q2 < - 2
		OR average_diff_q3_q2 > 2
	)
SELECT c.constructorRef
	,v.avg_diff_q2_q1
	,ROUND(AVG(diff_q3_q2), 2) avg_diff_q3_q2
	,v.avg_diff_q2_q1 + ROUND(AVG(diff_q3_q2), 2) avg_diff_q3_q1
FROM qualifying_2022 q
JOIN constructors c ON q.constructorId = c.constructorId
JOIN view_avg_diff_q2_q1_2022 v ON v.constructorRef = c.constructorRef
LEFT JOIN cte_exclude_2 e ON e.exclude = q.raceId
LEFT JOIN (
	SELECT convert_time(q3) - convert_time(q2) diff_q3_q2
		,driverRef
		,raceId
		,constructorId
	FROM qualifying_2022
...
WITH cte_sum AS (
		...
		SELECT SUM(ca.avg_q1) / COUNT(ca.avg_q1) sum_avg_q1
			,ca.constructorRef
			,MIN(SUM(ca.avg_q1) / COUNT(ca.avg_q1)) OVER () min_sum
		FROM cte_avg ca
		GROUP BY ca.constructorRef
		)

SELECT cs.constructorRef
	,ROUND(cs.sum_avg_q1 - min_sum, 2) diff_to_best_q1_constructor
FROM cte_sum cs
